...
import File from 'components/workspace/File';
...
export function fetchNode(src) {
  ...
  for (let i = 0; i < path.length; i++) {
    if (obj.file) {
      return { failed: 'notfolder', src: obj.src };
    }

    const holds = obj.holds;
    const j = holds.findIndex(item => item.name === path[i]);
    if (j === -1) {
      return { failed: 'intermediate', exists: obj, todo: path.slice(i) };
    }
...
function placeInFolder(holds, node) {
  ...
  if (holds[ind - 1] && holds[ind - 1].name === node.name) {
    return false;
  }
...
export function createModal(prompt, src, func, doSelect) {
  freezer.get().workspace.set({
    modal: schema.Modal(prompt, src, func, doSelect),
  });
}
